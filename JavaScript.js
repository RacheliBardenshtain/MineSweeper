...
function unload() {//שמירת נתוני המשחק שנסגר באמצע
    if (isOver == 0) {   // עצר באמצע משחק
        ...
        for (i = 0; i < x; i++) {
            mat2[i] = new Array(y);
            for (j = 0; j < y; j++) {
                if (document.querySelector('[data-row="' + i + '"]').children[j].getAttribute("data-visited") == "vi")//מספר גלוי
                    mat2[i][j] = 1;
                else if (document.querySelector('[data-row="' + i + '"]').children[j].classList.contains("flag"))//דגל
                    mat2[i][j] = 2;
                else
                    mat2[i][j] = 0;//משבצת שעוד לא נפתחה והיא לא דגל
            }
        }
